...
class DiffEmbedding(nn.Module):
    """
    Difference embedding module for encoding temporal differences.
    ...
    def __init__(self):
        ...
        # Compute the difference encodings once in log space

    def forward(self, x):
        """
        Forward pass of the difference embedding module.

        Args:
            x (torch.Tensor): The input tensor.

        Returns:
            torch.Tensor: The difference encodings corresponding to the input tensor.
        """
        shifted_x = torch.zeros_like(x)
        shifted_x[:, 1:, :] = x[:, :-1, :]
        x = shifted_x - x
        return self.de[:, :x.size(1)]
